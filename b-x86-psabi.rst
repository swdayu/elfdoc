...
基本类型： ::
    ...
    short/signed short/unsigned short               2   2       twobyte
    int/signed int/enum/unsigned int                4   4       fourbyte
    long/signed long/unsigned long                  4   4       fourbyte
    long long/signed long long/unsigned long long   8   4       eightbyte
...
%st(0)
    浮点返回值位于浮点寄存器栈的栈顶寄存器中，浮点寄存器中的单精度和双精度值的表示是相同
...
类型，由所有通过栈传递的参数组成的结构体必须对齐到32字节或64字节地址处。
...
者负责传递内存返回位置的地址，这个地址通过第一个隐式参数传递给被调函数。该地址必须满足数
据对象的地址对齐要求。被调函数需要将返回值写到给定的内存地址，并且要负责在返回前将隐式参
数从栈中弹出并保存到 %eax 寄存器中。函数调用者可能在函数返回后获取 %eax 的值来当作返回
值的引用。
